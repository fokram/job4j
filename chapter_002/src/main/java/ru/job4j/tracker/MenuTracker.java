...
public class MenuTracker {
    ...
    public final class ShowItems extends BaseAction {
        ...
        @Override
        public void execute(Input input, Tracker tracker) {
            output.accept("------------ Отображение всех заявок --------------");
    ...
    public final class FindItemById extends BaseAction {
        ...
        @Override
        public void execute(Input input, Tracker tracker) {
            output.accept("------------ Поиск заявки по идендификатору --------------");
    ...
    public final class DeleteItem extends BaseAction {
        ...
        @Override
        public void execute(Input input, Tracker tracker) {
            output.accept("------------ Deleting the item --------------");
            String id = input.ask("Введите id удаляемой заявки :");
            if (tracker.delete(id)) {
                output.accept(String.format("------------ Заявка %s успешно удалена-----------", id));
            } else {
                output.accept(String.format("------------ Заявка %s не найдена-----------", id));
            }
        }
    }
    ...
    public final class UpdateItem extends BaseAction {
        ...
        @Override
        public void execute(Input input, Tracker tracker) {
            output.accept("------------ Updating the item --------------");
            String id = input.ask("Введите id заменяемой заявки :");
            Item item = tracker.findById(id);
            if (item == null) {
                output.accept("Нет такой заявки.");
                return;
            }
            ...
            if (tracker.replace(id, item)) {
                output.accept(String.format("------------ Заявка %s успешно изменена-----------", item.getId()));
            }
            tracker.showItem(item);
        }
    }
